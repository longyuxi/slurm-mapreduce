...
from db import DB, INDEX, CROSS_DECOYS_BASE_FOLDER
...
KEY_PREFIX = 'adfr_cross_decoys_' # Prefix of every job, as appears in the Redis database
CLUSTER = 'CS' # or 'DCC'
...
def populate_db(rebuild_all_keys=False):
    ...
    for i, k in enumerate(tqdm(keys)):
        ...
        DB.hset(k, mapping={
            ...
            'error': 'False',
            'finished': 'False',
            'protein_file': f'{CROSS_DECOYS_BASE_FOLDER}/{i}/protein.pdb',
            'ligand_file': f'{CROSS_DECOYS_BASE_FOLDER}/{i}/ligand.pdb'
        })
